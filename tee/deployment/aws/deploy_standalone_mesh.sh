...
# This script creates self-contained versions of the coordinator and controller
# directly on the AWS instances to avoid workspace dependency issues
...
echo "Creating standalone controller code..."
...
async fn get_mesh_status(state: axum::extract::State<SharedState>) -> impl IntoResponse {
    let state_guard = state.lock().unwrap();
    ...
    (StatusCode::OK, Json(mesh_status))
}

async fn execute_contract(Json(req): Json<ExecutionRequest>) -> impl IntoResponse {
