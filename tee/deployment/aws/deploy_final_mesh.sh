...
# Final fixed implementation with all imports properly specified
...
echo "Creating final standalone controller code..."
...
async fn execute_contract(
    ...
    if params.len() >= 4 {
        ...
        if len > 0 && len <= 1024 && len + 4 <= params.len() {
            ...
            let _data = params[4..4+len].to_vec();
            println!("Parameter data: {:?}", _data);
        } else {
...
# Create cleanup script for services and files
echo "Creating cleanup script..."
cat > cleanup_mesh.sh << 'EOF'
#!/bin/bash
...
echo "Waiting for coordinator to start (10 seconds)..."
sleep 10
...
for i in {0..1}; do
  ...
  # Give a small pause between starting services to avoid overwhelming the coordinator
  sleep 2
done
...
curl -s http://${COORDINATOR_IP}:8080/tees

echo -e \"\\n\\n========== SGX Node Status ==========\"
curl -s http://${SGX_IPS[0]}:7070/mesh-status

echo -e \"\\n\\n========== SEV Node Status ==========\"
curl -s http://${SEV_IPS[0]}:7070/mesh-status
...
echo -n -e '\\x0A\\x00\\x00\\x00TESTPAYLOAD' > /tmp/test_payload_length.bin
PAYLOAD_LENGTH_PREFIX=\$(base64 -w 0 /tmp/test_payload_length.bin)
curl -s -X POST -H \"Content-Type: application/json\" \\
  -d \"{\\"contract_id\\":\\"test-contract\\",\\"function\\":\\"test-function\\",\\"parameters\\":\"\$PAYLOAD_LENGTH_PREFIX\"}\" \\
  http://${SGX_IPS[0]}:7070/execute

echo -e \"\\n\\nTesting direct format...\"
# Create a direct payload without length prefix
echo -n -e 'DIRECTPAYLOAD' > /tmp/test_payload_direct.bin
PAYLOAD_DIRECT=\$(base64 -w 0 /tmp/test_payload_direct.bin)
curl -s -X POST -H \"Content-Type: application/json\" \\
  -d \"{\\"contract_id\\":\\"test-contract\\",\\"function\\":\\"test-function\\",\\"parameters\\":\"\$PAYLOAD_DIRECT\"}\" \\
  http://${SGX_IPS[0]}:7070/execute

echo -e \"\\n\\n========== Cross-Attestation Test ==========\"
echo \"Testing SGX -> SEV attestation...\"
curl -s -X POST -H \"Content-Type: application/json\" \\
  -d \"{\\"contract_id\\":\\"cross-attestation\\",\\"function\\":\\"attest\\",\\"parameters\\":\"\$PAYLOAD_LENGTH_PREFIX\"}\" \\
  http://${SGX_IPS[0]}:7070/execute
...
ssh -o StrictHostKeyChecking=no -i ~/${KEY_NAME}.pem ubuntu@${COORDINATOR_IP} "sudo chmod +x /opt/rhombus/check_dual_tee_mesh.sh"
